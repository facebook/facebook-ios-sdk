...
#import "CustomLoggingViewController.h"

#import <FBSDKCoreKit/FBSDKCoreKit.h>

#import "AlertControllerUtility.h"

@interface CustomLoggingViewController ()

@property (weak, nonatomic) IBOutlet UITextField *eventField;

@end

@implementation CustomLoggingViewController

- (IBAction)logCustomEvent:(id)sender
{
  UIAlertController * alertController;
  if (_eventField.text.length <= 0) {
    alertController = [AlertControllerUtility alertControllerWithTitle:@"Invalid event" message:@"Event cannot be empty."];
    [self presentViewController:alertController animated:YES completion:nil];
  } else {
    [FBSDKAppEvents logEvent:_eventField.text];
