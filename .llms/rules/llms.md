...
## Diff Summaries Are Published to Public GitHub

This repo is mirrored to GitHub, and ShipIt exports the **whole** commit message.
The `Summary` and `Test Plan` you write in Phabricator appear verbatim in public
history. Treat every word as a public release: once pushed it is cloned, forked,
and indexed, and amending the diff later does not retract what was mirrored.

Write the summary for an external reader. Describe *what changed and why* in
terms of the public API and the code in this repo. Do not paste in anything whose
audience is internal.

Keep out of `Summary` and `Test Plan`:

- **Internal identifiers** — task numbers (`T…`), SEV numbers, and internal-only
  diff references. Say "tracked internally" or restate the substance instead.
- **Internal URLs** — `internalfb.com`, `fburl.com`, Phabricator, wikis,
  dashboards, Scuba/ODS links. An external reader cannot open them anyway.
- **Unreleased plans** — launch dates, rollout percentages, upcoming products,
  deprecations that have not been announced.
- **Gating and config names** — GateKeepers, JustKnobs, sitevars, experiment
  names, and their values.
- **Infrastructure detail** — hostnames, service names, capacity, internal
  metrics, and raw log excerpts that embed any of the above.
- **People** — names, unixnames, and org structure in the prose. (The
  `Reviewers` / `Subscribers` / `Differential Revision` trailers are generated by
  `jf` and are exported regardless; that is expected. Everything you type is not.)

CI signal names and script paths that already live in this repo (for example
`ios-sdk-unit-tests`, `ci/unit_tests.py`) are fine — they are public here.

Before `jf submit`, scan the message:

```bash
git log -1 --format=%B | grep -inE 'T[0-9]{6,}|internalfb|fburl|phabricator|\bGK\b|justknob|sitevar|SEV[0-9]'
```

Hits in the body need rewording; hits in the generated trailers are expected.
