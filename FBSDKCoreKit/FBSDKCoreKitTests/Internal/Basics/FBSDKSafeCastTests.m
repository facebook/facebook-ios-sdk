...
{
  ClassA *a = [ClassA new];
  XCTAssertNil(FBSDK_CAST_TO_CLASS_OR_NIL(a, ClassB),
               "Casting from a known class to a non-matching class should fail and return nil");

  id idA = a;
  XCTAssertNil(FBSDK_CAST_TO_CLASS_OR_NIL(idA, ClassB),
...
{
  ClassA *a = [ClassA new];
  XCTAssertEqual(FBSDK_CAST_TO_CLASS_OR_NIL(a, ClassA), a,
                 "Casting from a known class to a matching class should return the same instance of that class");

  id idA = a;
  XCTAssertEqual(FBSDK_CAST_TO_CLASS_OR_NIL(idA, ClassA), idA,
...
- (void)testCastingToNonConformingProtocol
{
  ...
  XCTAssertNil(FBSDK_CAST_TO_PROTOCOL_OR_NIL(a, ProtocolB),
               "Should not return an object if it does not conform to the stated protocol");

  id idA = a;
  XCTAssertNil(FBSDK_CAST_TO_PROTOCOL_OR_NIL(idA, ProtocolB),
               "Should not return an object if it does not conform to the stated protocol");
}
