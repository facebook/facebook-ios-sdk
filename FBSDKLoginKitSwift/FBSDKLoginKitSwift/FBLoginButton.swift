...
public extension FBLoginButton {
   ...
   */
  convenience init(frame: CGRect = .zero, readPermissions: [ReadPermission] = [.publicProfile]) {
    self.init(frame: frame)
    self.readPermissions = readPermissions.map { $0.permission.name }
  }
   ...
   */
  convenience init(frame: CGRect = .zero, publishPermissions: [PublishPermission] = [.publishActions]) {
    self.init(frame: frame)
    self.publishPermissions = publishPermissions.map { $0.permission.name }
  }
}
