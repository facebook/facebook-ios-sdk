...
@testable import FBSDKCoreKitSwift
...
public enum LoginResult {
  ...
  internal init(result: LoginManagerLoginResult?, error: Error?) {
    ...
    let granted: Set<Permission> = Set(result.grantedPermissions.map { Permission(name: $0) })
    let declined: Set<Permission> = Set(result.declinedPermissions.map { Permission(name: $0) })
...
public extension LoginManager {
   ...
   */
  public convenience init(loginBehavior: LoginBehavior = .native, defaultAudience: DefaultAudience = .friends) {
    self.init()
  ...
  public func logIn(readPermissions: [ReadPermission] = [.publicProfile],
    ...
    let permissions: [String] = readPermissions.map { $0.permission.name }
    self.logIn(readPermissions: permissions, from: viewController, handler: sdkCompletion(completion))
  }
  ...
  public func logIn(publishPermissions: [PublishPermission] = [.publishActions],
                    viewController: UIViewController? = nil,
                    completion: LoginResultBlock? = nil) {
    let permissions: [String] = publishPermissions.map { $0.permission.name }
    self.logIn(publishPermissions: permissions, from: viewController, handler: sdkCompletion(completion))
  }
