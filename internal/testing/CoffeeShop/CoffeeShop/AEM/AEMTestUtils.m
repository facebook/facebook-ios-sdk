  ...
  deeplinkType:(AEMDeeplinkUrlType)deeplinkType
{
  ...
  NSDictionary<NSString *, NSString *> *data = @{@"acs_token" : @"test_acstoken",
...
+ (void)reset:(UITextView *)console
{
  if (campaignId.length > 0) {
    console.text = [NSString stringWithFormat:@"Jumped From Campaign: %@\t\t", campaignId];;
  }
}
...
{
  [FBSDKSettings.sharedSettings setLoggingBehaviors:[NSSet setWithArray:@[@"developer_errors", @"network_requests"]]];
}
